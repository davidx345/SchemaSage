...
interface CrossDatasetRelationshipsProps {
  datasets: SchemaTable[][];
  onApply?: (relationships: Relationship[]) => void;
}

export function CrossDatasetRelationships({ datasets, onApply }: CrossDatasetRelationshipsProps) {
  ...
  const fetchSuggestions = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/schema-detection/relationships/cross-dataset", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ datasets }),
      });
      if (!res.ok) throw new Error("Failed to fetch cross-dataset suggestions");
  ...
  return (
    <Card className="p-4 my-4">
      <div className="flex items-center justify-between mb-2">
        <h3 className="font-semibold">Cross-Dataset Relationship Suggestions</h3>
        <Button onClick={fetchSuggestions} disabled={loading} size="sm">
          {loading ? "Loading..." : "Suggest Cross-Dataset Relationships"}
        </Button>
      </div>
